...
`github.com/AtomiCloud/diene.go-api-engine`, materialized from the `go-lib`
...
`github.com/AtomiCloud/diene.go-auth-engine` and
...
The public packages are `lib/apiengine` and `lib/wire`, plus the
consumer-facing `testhelper`. This library binds no infrastructure of its own,
so it ships no `adapters/` tree and its integration tier is a documented no-op;
the outbound HTTP transport and the per-backend token retriever are both seams
the consumer binds. The module has no `main` or
