...
- Physical instance ids arrive as an already-minted **pair**, and the chart records both halves rather than deriving either. `global.instance.original` is the unchanged repository-qualified id — it may be far longer than one DNS label, up to 253 bytes, and is refused only for whitespace, control bytes, or characters outside `A-Za-z0-9._:/-`. `global.instance.label` is the authorized minter's stable DNS-1123 label for that id, at most 63 bytes, and is the **only** half that becomes a hostname segment (`instance.hostname`). Both are stamped, distinct, as `<prefix>/instance-original` and `<prefix>/instance-label`, which is what makes a hash-shortened label reversible and collision-auditable from the object itself. The two are recorded together or not at all: a missing key refuses at the generated schema's `required` list, an emptied half refuses at the helper as `InstancePairIncomplete`, and both empty simply record no instance.
  - "Record no instance" means the two annotation keys are **absent**, not present-and-empty, and it means that on **every** object — wrapper-owned and pinned-dependency alike. That symmetry is what lets a reader treat presence of `<prefix>/instance-label` as "this object belongs to a physical instance"; an empty-valued key would false-positive that read on exactly the objects the wrapper does not render itself. Getting it took a second deliberate deviation from the pinned upstream emission, because `upstream.global.annotations` is a values **map** and a values file cannot make a map key conditional; see [Upstream metadata interface](#upstream-metadata-interface).
...
- `chart/templates/_helpers.tpl` **redefines** `bjw-s.common.lib.metadata.globalLabels` and `bjw-s.common.lib.metadata.globalAnnotations`. Helm's template namespace is global and `sortTemplates` parses deeper paths first, so the wrapper's shallow `templates/_helpers.tpl` is parsed after the dependency's `charts/upstream/charts/common/templates/lib/metadata/*.tpl` and a same-named definition replaces it for the whole render — including when the wrapper is rendered from a packaged archive, whose relative depths are identical. The replacements reproduce the upstream emission byte-for-byte and deviate in exactly **two** deliberate ways:
  1. **The key is `tpl`-rendered too**, so `global.labelPrefix` reaches an upstream key exactly as it reaches a wrapper-owned one. Every rendered key is then judged as a Kubernetes qualified name, so a computed key the API server would reject refuses at render instead of at apply.
  2. **An entry whose rendered value is empty is omitted**, rather than emitted with an empty value. A values map always carries its keys, so upstream's emission cannot express "this object records nothing" — but the wrapper's own `annotations` helper can, and does, for the recorded instance pair. Without this the dependency's two objects would wear `<prefix>/instance-original` and `-label` with empty values while the other thirteen wore neither, so one release would answer "does this object belong to a physical instance" two different ways. Key validation still runs on every key that is emitted; only the empty-valued entry is skipped, and no projection slot or reloader annotation is ever empty, so none is dropped.
...
The gate then holds both object families to the same rule under both the default prefix and an override: every slot of the stack's projection present and exact in labels **and** annotations, the stack's recorded instance arity in the annotations — the pair present and byte-exact, or, for the empty-pair stack below, neither key present at all — and, under an override, no key surviving under the old prefix. Wrapper-owned objects project module `api` and the dependency's objects project module `upstream`; both lists are named, so an object that stops rendering is reported missing instead of quietly shrinking coverage. `probes/wrapper-lpsm-labels.ts` sabotages exactly one upstream label key back to a literal `atomi.cloud/…`, which leaves every default-prefix render byte-identical and can only be caught by the override runs.

A seventh stack renders with `global.instance.preview.enabled=true`, where both recorded halves are the receipt-bound petname, and holds the named upstream Deployment and Service to the same recorded identity as every wrapper-owned object.

An eighth stack renders with `--set-string global.instance.original= --set-string global.instance.label=` — the schema-valid "no minted physical instance" shape — and requires **neither** annotation key on either object family, while asserting the same exact projection, the same named object list, and the same hook isolation as every other stack. Absence is its own explicit arity of the assertion, not a present-pair check handed two empty expected values: `""` would be satisfied by an object that emits the key with an empty value, which is the very divergence this stack exists to catch. In both arities the validator first requires the exact sorted key set `["instance-label", "instance-original"]`, then requires non-empty strings for a present pair or nulls for an absent pair. Four destroying-path controls remove each half in each mode; if either one-key map is accepted, `labels` itself fails. The present-pair/absent-pair direction checks remain too, so the eighth stack cannot be quietly turned into a weaker copy of the other seven.

Three chart-sabotage controls run beside them, each against a throwaway copy of the chart so the working tree is never touched, and each demanding its **named** offender rather than merely some offender:

- restoring `diene-helm-wrapper.labels` on the migration pod must report `selects the Job/wrapper-migration pod`. That sabotage also resets the hook's workload name and drops its component, so three offender families fire at once; requiring the selector sentence specifically is what makes a `selects` comparison hardwired to false fail the control instead of coasting on the other two;
- hard-coding the dependency's instance annotations back to the physical pair must stay invisible in physical mode — it is byte-identical there, exactly as the defect was — and must report the upstream Deployment's `instance-original` mismatch on the preview stack;
- emitting an empty-valued upstream global metadata entry instead of omitting it must stay invisible in all seven recorded-pair stacks and must report the upstream Deployment's `instance-original annotation="" want=absent` on the empty-pair stack.
...
A hook is **not** the primary workload, so it does not wear the primary workload's selector identity. Every primary Service selector and the Deployment's own `matchLabels` are exactly `selectorLabels`: the primary resource name, release instance, and fixed `app.kubernetes.io/component: primary`. The component is deliberately not derived from `serviceTree.module`. `diene-helm-wrapper.hookLabels` keeps everything that makes a hook a service-tree object (the full projection under the prefix in force, chart, version, managed-by, the recorded instance pair in the annotations) but keeps its hook token as `app.kubernetes.io/component` — `migration` for this Job. Therefore the supported `serviceTree.module=migration` plus `fullnameOverride=wrapper-migration` configuration can intentionally give the primary workload and hook the same name without allowing any primary selector to capture the hook pod.

`scripts/validate/helm-wrapper.sh labels` asserts that directly rather than by inspection: every named selector-bearing object must render, must carry a non-empty selector, and must fail to select the hook pod, while the hook pod itself must still carry the exact projection and instance annotations — so the check cannot pass by matching nothing. It repeats that assertion for the valid same-name migration configuration, with its renamed primary Service and Deployment listed explicitly. Because the default stack holds, `scripts/validate/helm-wrapper-k3d.sh` selects pods by the Deployment's name/instance label subset and no longer filters `batch.kubernetes.io/job-name` out of the result; the workaround existed only to hide this defect.
