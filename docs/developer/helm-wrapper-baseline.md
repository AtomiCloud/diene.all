...
An eighth stack renders with `--set-string global.instance.original= --set-string global.instance.label=` — the schema-valid "no minted physical instance" shape — and requires **neither** annotation key on either object family, while asserting the same exact projection, the same named object list, and the same hook isolation as every other stack. Absence is its own explicit arity of the assertion, not a present-pair check handed two empty expected values: `""` would be satisfied by an object that emits the key with an empty value, which is the very divergence this stack exists to catch. The arity is guarded both ways — a present-pair assertion handed no recorded bytes and an absence assertion handed real bytes both refuse — so the eighth stack cannot be quietly turned into a weaker copy of the other seven.

Three sabotage controls run beside them, each against a throwaway copy of the chart so the working tree is never touched, and each demanding its **named** offender rather than merely some offender:
