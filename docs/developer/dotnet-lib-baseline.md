...
- `AtomiCloud.Diene.E2e` contains the in-process and Garden SIT drivers;
- `AtomiCloud.Diene.E2e.TestHelper` contains black-box response assertions and references
  the main package.
...
- `pls test:unit` measures the real `AtomiCloud.Diene.E2e` assembly plus the
...
- `pls test:int` drives the demo through `WebApplicationFactory` and measures
  only `[App*]*`; the global `Program` bootstrap remains excluded.
...
- `skills/diene-dotnet-e2e-usage/` to the materialized library's namespaced
  usage skill.
