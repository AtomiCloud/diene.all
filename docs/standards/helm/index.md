...
Chart linting is available directly and also runs in `pls lint`. Generated chart
docs are enforced by the `helm-docs` pre-commit hook.
...
1. The reusable workflow uses `AtomiCloud/actions.setup-nix` and runs inside `nix develop .#cd`,
   ...
   `nscloud-cache-tag-atomi-nix-store-cache-linux-amd64` (one cache for all Nix jobs — no per-service keys),
