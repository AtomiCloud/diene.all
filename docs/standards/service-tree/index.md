...
makes that unnecessary). This branch publishes no image artifacts, so it carries no
caller job that names one; a descendant that adds an image axis passes those names as
`with:` inputs on its own caller jobs.
