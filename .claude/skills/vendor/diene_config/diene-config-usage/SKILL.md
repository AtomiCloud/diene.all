...
Retain each exact `ConfigBlock<T>` instance you pass to `ConfigSchema` and use
that same object for `slice`, `optionalSlice`, and `hasSlice`. Membership is
identity-based: reconstructing a same-key lookalike does not grant access to the
composed block's decoded value.
