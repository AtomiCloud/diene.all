...
- Return ordinary `(T, error)` pairs. Expected failures must be problem-typed
  errors from `github.com/AtomiCloud/diene.go-errors-problems/lib/problem`.
...
`Enqueue…Result` methods, using `problem.NewError` for expected fault paths.
