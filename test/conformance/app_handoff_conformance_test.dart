...
void main() {
  ...
  final Map<String, Object?> root =
      _obj(jsonDecode(File('test/fixtures/c0/identity.json').readAsStringSync()));
  ...
  const AppHandoffUser primary =
      AppHandoffUser(sub: 'u1', primaryEmail: 'a@b.com');
  ...
  test('constants and routes match the frozen fixture', () {
    // Carrier prefix.
    expect('$appHandoffCarrierScheme:$appHandoffCarrierVersion:',
        c['carrierPrefix']);
    ...
    expect(
      const AppHandoffRedeemResponse(token: 't', email: 'e').toJson().keys.toSet(),
  ...
  test('nonce codec accepts/rejects per the frozen fixture alphabet', () {
    ...
    final String paddedNonce =
        (paddedVector['text']! as String).substring(prefix.length);
  ...
  test('valid redeem requests yield a token per the frozen fixture', () async {
    ...
    for (final Object? raw in _list(requests['valid'])) {
      ...
      final (int status, Map<String, Object?> resp) =
          await redeem(jsonEncode(body));
