  ...
  group('ProblemRegistry', () {
    test('registers, looks up, and requires types', () {
      final registry = ProblemRegistry(_portal)
        ..register(const ProblemType(id: 'x', title: 'X', version: 'v1', status: 400));

      expect(registry.lookup('x')?.id, 'x');
      expect(registry.require('x').id, 'x');
      expect(() => registry.require('missing'), throwsA(isA<UnknownProblemTypeError>()));
    });
    ...
    test('rejects duplicate ids', () {
      ...
      expect(
        () => registry.register(const ProblemType(id: 'dup', title: 'D2', version: 'v1')),
    ...
    test('typeUriFor builds the URI through the single-source builder', () {
      final registry = ProblemRegistry(_portal)
        ..register(const ProblemType(id: 'entity-not-found', title: 'X', version: 'v1'));
      expect(
        registry.typeUriFor(registry.require('entity-not-found')),
    ...
    test('entries preserve insertion order', () {
      ...
      expect(registry.entries.map((ProblemType t) => t.id).toList(), ['a', 'b']);
    });
  });
