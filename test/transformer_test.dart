  ...
  group('fromObject', () {
    ...
    test('maps a known id on a Map to the registry type', () {
      ...
      final error = <String, Object?>{
        'problemId': 'entity-not-found',
        'data': <String, Object?>{'resource': 'user'},
      };
      // Act
      final p = fromObject(error, options: TransformOptions(registry: registry, portal: _portal));
      // Assert
      expect(p.status, 404);
      expect(p.recoverable, false);
      expect(p.type, contains('/v1/entity-not-found'));
      expect(p.data, {'resource': 'user'});
    });

    test('falls back to an uncatalogued problem for an unknown value', () {
      final p = fromObject(StateError('boom'), options: const TransformOptions(portal: _portal));
      expect(p.status, 500);
      expect(p.recoverable, false);
    ...
    test('falls back to uncatalogued when no registry is supplied', () {
      final p = fromObject(<String, Object?>{'problemId': 'entity-not-found'});
      expect(p.type, contains('/$uncataloguedProblemId'));
    });

    test('falls back to uncatalogued when the registry does not know the id', () {
      final registry = ProblemRegistry(_portal);
      final p = fromObject(
        <String, Object?>{'problemId': 'nope'},
