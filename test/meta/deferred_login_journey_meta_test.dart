...
void main() {
  ...
  test('valid android carrier is redeemed into a token', () async {
    ...
    final DeferredLoginJourney driver =
        DeferredLoginJourney(baseUrl: server.baseUrl, platform: 'android');
    // Act.
    final DeferredLoginResult result = await driver.redeemCarrier(carrier);
    driver.close();
    // Assert.
    expect(result.outcome, DeferredLoginOutcome.redeemed);
  ...
  test('absent/invalid carrier falls back to interactive login', () async {
    final DeferredLoginJourney driver =
        DeferredLoginJourney(baseUrl: server.baseUrl, platform: 'ios');
    final DeferredLoginResult result = await driver.redeemCarrier('garbage');
    driver.close();
    expect(result.outcome, DeferredLoginOutcome.interactiveFallback);
