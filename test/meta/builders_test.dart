...
void main() {
  group('aProblem', () {
    test('mints a type URI via problemTypeUri (never hand-formatted)', () {
      final p = aProblem(id: 'entity-not-found', version: 'v1');
      expect(p.type, 'https://local.atomi.cloud/docs/local/flutter/app/core/v1/entity-not-found');
    });

    test('honours status/recoverable/data overrides', () {
      final p = aProblem(status: 409, recoverable: true, data: <String, Object?>{'k': 'v'});
      expect(p.status, 409);
    ...
    test('accepts a custom portal', () {
      ...
      expect(aProblem(portal: portal, id: 'x').type, 'https://h/docs/l/p/s/m/v1/x');
    });
  });
  ...
  group('aCatalogEntry', () {
    test('mints a single-source type URI and renders valid CR content', () {
      final entry = aCatalogEntry(id: 'validation-error', status: 400, recoverable: true);
      expect(entry.typeUri, contains('/v1/validation-error'));
    ...
    test('carries declared endpoints', () {
      final entry = aCatalogEntry(
        id: 'x',
        endpoints: const <CatalogEndpoint>[CatalogEndpoint(method: 'GET', path: '/x')],
      );
