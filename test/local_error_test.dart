...
void main() {
  group('LocalError', () {
    test('wraps message + stacktrace into data and routes through the builder', () async {
      // Arrange
      ...
      // Act
      final problem = await LocalError(sink, portal: portal).wrap(StateError('broken'), stack);
      // Assert
      ...
      expect(problem.type, 'https://docs.raichu.cluster.atomi.cloud/docs/raichu/dotnet/user/api/v1/local-error');
      expect(sink.captured, same(problem));
    });

    test('uses the localError fallback portal by default', () async {
      final problem = await const LocalError(NoopErrorSink()).wrap('x', StackTrace.empty);
      expect(problem.type, 'https://local.atomi.cloud/docs/local/flutter/app/core/v1/local-error');
    });
  });

  test('NoopErrorSink captures nothing (no throw)', () async {
    const sink = NoopErrorSink();
    await expectLater(sink.capture(const Problem(type: 't', title: 'T', status: 500)), completes);
  });
}
