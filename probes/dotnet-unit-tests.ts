...
export default {
  ...
  probes: [
    ...
    },
    {
      ...
      async run(repo: any) {
        await repo.patch('UnitTest/ConfigKeyTests.cs', {
          find: '        ConfigKey.Segment(spelling).Should().Be(canonical);',
          replace: '        ConfigKey.Segment(spelling).Should().NotBe(canonical);',
        });
