...
const DRY_RUN_HAS_SKILL =
  'nix develop .#ci --no-write-lock-file -c bash -lc \'cd packages/diene_core_utils && out=$(dart pub publish --dry-run --skip-validation 2>&1) && printf "%s\\n" "$out" | grep -F -q diene-core-utils-usage\'';
...
export default {
  ...
  probes: [
    ...
    },
    {
      ...
      expectedImpact: [],
      async run(repo: any) {
        const pubignore = 'packages/diene_core_utils/.pubignore';
