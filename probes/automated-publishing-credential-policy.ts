...
const TAG_PIN_SCRIPT: Array<string | RegExp> = [
  ...
  'git checkout "refs/tags/${TAG}" -- packages/diene_api_engine',
  'git --no-pager diff --stat "refs/tags/${TAG}" -- packages/diene_api_engine',
];
...
const BYPASSES: Array<{ label: string; build: (source: string) => string }> = [
  ...
  },
  {
    ...
    build: source => source.replace('git checkout "refs/tags/${TAG}" -- packages/diene_api_engine\n', ''),
  },
