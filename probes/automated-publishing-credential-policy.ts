...
const TAG_PIN_SCRIPT: Array<string | RegExp> = [
  ...
  'git checkout "refs/tags/${TAG}" -- packages/diene_auth_engine',
  'git --no-pager diff --stat "refs/tags/${TAG}" -- packages/diene_auth_engine',
];
...
const BYPASSES: Array<{ label: string; build: (source: string) => string }> = [
  ...
  },
  {
    ...
    build: source =>
      insertAfterConfigure(
        source,
        '      - name: Extra publish\n        run: cd packages/diene_auth_engine && flutter pub publish --force\n',
      ),
  },
  {
  ...
  },
  {
    ...
    build: source => source.replace('git checkout "refs/tags/${TAG}" -- packages/diene_auth_engine\n', ''),
  },
