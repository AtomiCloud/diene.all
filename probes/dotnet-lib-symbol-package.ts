...
export default defineGate({
  ...
  mutation: {
    ...
    async run(repo: any) {
      ...
      await repo.exec(
        'package="$(find artifacts/package -maxdepth 1 -name \'AtomiCloud.Diene.Interfaces.TestHelper.*.nupkg\' -print -quit)"; symbols="${package%.nupkg}.snupkg"; cp "${package}" "${symbols}"',
      );
      await expectRed(
        repo,
        'nix develop .#ci -c ./scripts/validate/dotnet-package.sh symbols',
