...
const GUARD = "hashFiles('packages/diene_auth_engine/lib/test_helper.dart') != ''";
  ...
  probes: [
    {
      ...
      async run(repo: any) {
        const result = await repo.exec(
          "nix develop .#ci --no-write-lock-file -c bash -lc 'rm -rf packages/diene_auth_engine/coverage/meta && TEST_HELPER_PATH=lib/__probe_absent_helper__.dart ./scripts/ci/test.sh meta coverage'",
          { timeoutMs: 240000 },
        );
