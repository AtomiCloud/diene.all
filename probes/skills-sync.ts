...
export default {
  ...
  probes: [
    ...
    {
      name: 'baseline-skills-sync-hosted-pub-green',
      description:
        'The universal skills synchronizer decodes an encoded hosted Pub file URI and vendors the package skills.',
      kind: 'baseline',
      async run(repo: any) {
        await withCleanProbeState(repo, probeCleanTargets, async () => {
          // The leading clean deliberately removes .dart_tool. Recreate it in
          // the execution phase, matching the suite's artifact-free sandbox
          // contract rather than copying resolver state into every fork.
          await expectGreen(repo, offlinePubGetCommand, 'skills-sync-hosted-pub-setup');

          const cwd = await repo.exec('pwd');
          if (cwd.exitCode !== 0) {
            throw new Error(`skills-sync-hosted-pub: failed to resolve sandbox root: ${cwd.stderr || cwd.stdout}`);
          }

          await repo.write(`${hostedFixtureDir}/skills/hosted/SKILL.md`, '# Hosted probe\n');
          const packageConfig = JSON.parse(await repo.read(packageConfigPath));
          ...
          await expectGreen(repo, syncCommand, 'skills-sync-hosted-pub');
          if ((await repo.glob(`${vendorDir}/diene_hosted_probe/hosted/SKILL.md`)).length !== 1) {
            throw new Error('skills-sync-hosted-pub: hosted skills were not vendored');
          }
        });
      },
    },
