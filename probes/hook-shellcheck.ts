...
// The mutation arm needs a tracked shell script carrying a finding, and any one will do.
// It used to append the finding to scripts/validate/workflows.sh because that file "could
// not be lost"; subjects have been deleted out from under this arm twice already, so it
// owns its subject here in the directory this probe already owns and cleans up.
const findingFixture = `${sourceFixtureDir}/unquoted-expansion.sh`;
const findingBody = `#!/usr/bin/env bash
set -euo pipefail

# Deliberate SC2086: an unquoted expansion. Written, staged and removed by the arm.
echo $HOME/$UNQUOTED
`;
...
export default {
  ...
  probes: [
    ...
    },
    {
      ...
      expectedImpact: [],
      async run(repo: any) {
        // Any tracked shell script carrying a finding proves the same thing - that the
        // hook reads real repository shell and refuses incorrect shell - so the arm plants
        // its own rather than borrowing a file another change may remove.
        await repo.write(findingFixture, findingBody);
        try {
          // The gate runs --all-files, which reads TRACKED files, so an unstaged fixture
          // would leave the hook inspecting exactly what it already inspected.
          const staged = await repo.exec(`git add ${findingFixture}`);
          if (staged.exitCode !== 0) {
            throw new Error(`could not stage the shellcheck fixture: ${staged.stderr || staged.stdout}`);
          }
          ...
          // unrelated reason, or by one inspecting a smaller set of files, so the finding
          // itself has to appear: SC2086 is ShellCheck's code for the unquoted expansion
          // this arm plants.
