...
const consumePublishedPackages =
  'version=$(xmlstarlet sel -t -v "/Project/PropertyGroup/Version" Version.props) && consumer=$(mktemp -d) && dotnet new console -o "$consumer" && dotnet add "$consumer" package AtomiCloud.Diene.Config --version "$version" --source https://api.nuget.org/v3/index.json && dotnet add "$consumer" package AtomiCloud.Diene.Config.TestHelper --version "$version" --source https://api.nuget.org/v3/index.json && dotnet build "$consumer" -c Release; rc=$?; if [ -n "${consumer:-}" ]; then rm -rf "$consumer"; fi; exit "$rc"';
