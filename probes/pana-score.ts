...
const PANA =
  "nix develop .#ci --no-write-lock-file -c bash -lc 'cd packages/diene_core_utils && dart doc --dry-run && dart run pana --no-dartdoc --exit-code-threshold 0 .'";
...
export default {
  ...
  probes: [
    ...
    },
    {
      ...
      expectedImpact: [],
      async run(repo: any) {
        await repo.patch('packages/diene_core_utils/pubspec.yaml', {
          find: 'description:',
          replace: '#description:',
        });
