  ...
  probes: [
    {
      ...
      description: 'flutter pub get resolves the workspace and writes a package config',
      kind: 'baseline',
      async run(repo: any) {
        await expectGreen(
          repo,
          "nix develop .#ci --no-write-lock-file -c bash -lc 'flutter pub get --offline || flutter pub get'",
