  ...
  probes: [
    {
      ...
      description: 'The observability and Grafana dashboard standards, five thin skills, and their assets exist.',
      kind: 'baseline',
      async run(repo: any) {
        for (const artifact of requiredArtifacts) {
          if ((await repo.glob(artifact)).length !== 1) {
            throw new Error(`missing observability payload artifact: ${artifact}`);
          }
        }
      },
    },
