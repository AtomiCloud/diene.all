  ...
  probes: [
    {
      ...
      async run(repo: any) {
        await expectGreen(
          repo,
          "nix develop .#ci --no-write-lock-file -c bash -lc 'dart --version && gitlint --version && cd packages/diene_dart_lib && dart run pana --help >/dev/null && dart run coverage:format_coverage --help >/dev/null 2>&1 && dart run dart_code_linter:metrics --help >/dev/null 2>&1'",
          'dart-tool-inventory',
        );
      },
    },
