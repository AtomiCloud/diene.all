  ...
  probes: [
    {
      ...
      async run(repo: any) {
        await expectGreen(
          repo,
          'nix develop .#ci --no-write-lock-file -c bash -lc \'dart --version >/dev/null 2>&1 && gitlint --version >/dev/null 2>&1 && cd packages/diene_problems && dart run pana --help 2>&1 | grep -qiE "usage|pana" && dart run coverage:format_coverage --help >/dev/null 2>&1 && dart run dart_code_linter:metrics --help >/dev/null 2>&1\'',
          'dart-tool-inventory',
        );
      },
    },
