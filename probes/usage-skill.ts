  ...
  probes: [
    {
      ...
      kind: 'baseline',
      async run(repo: any) {
        const source = await repo.read('skills/diene-standard-config-usage/SKILL.md');
        const frontmatter = source.match(/^---\n([\s\S]*?)\n---/);
        if (!frontmatter) {
          throw new Error('usage skill frontmatter is missing');
        }
        const name = frontmatter[1].match(/^name:\s*(.+)$/m)?.[1]?.trim();
        if (name !== 'diene-standard-config-usage') {
          throw new Error(`usage skill name mismatch: ${name ?? 'missing'}`);
        }
        if ((await repo.glob('skills/diene-standard-config-usage/patterns.md')).length !== 1) {
