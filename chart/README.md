...
| labelPrefix | string | `"atomi.cloud"` | Prefix used by every service-tree label helper. |
| reloader | object | `{"enabled":true}` | Reloader opt-in baked onto every cert-manager workload. Set false to opt out everywhere via overlay. |
...
| upstream.global | object | `{"commonLabels":{"atomi.cloud/layer":"1","atomi.cloud/module":"certs","atomi.cloud/platform":"sample","atomi.cloud/service":"sulfur"}}` | LPSM identity labels applied to every chart-rendered cert-manager resource. |
