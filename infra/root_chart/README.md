...
Runtime app chart for the diene go-consumer worker — worker Deployment with dependency-blind exec probes, db-init pre-sync Job, ExternalSecret, and the vendored config ConfigMap
...
| Resource                                                | Why                                                                   |
| ------------------------------------------------------- | --------------------------------------------------------------------- |
| `Deployment` `goconsumer-worker`                        | the long-running consume/produce loop                                 |
| `Job` `goconsumer-dbinit`                               | one-shot pre-sync migration/seed entry point                          |
| `ConfigMap` `goconsumer-config` / `-dbinitconfig`       | the vendored application config YAMLs                                 |
| `ExternalSecret` `goconsumer-secrets`                   | materializes the service secret from the platform SecretStore         |
...
(`/go-consumer health`). `health` inspects worker heartbeat and internal state
only. A postgres/redis/S3 blip must degrade the service, never evict every pod.
Dependency reachability belongs to `db-init` and to alerting.

Because the image is `distroless/static` there is no shell in the container, so
the probe command is the **compiled binary itself** — never a `sh -c` wrapper.

### The heartbeat path is the chart's business

`containerSecurityContext.readOnlyRootFilesystem` is on, so the only writable
location in the pod is the `tmp` `emptyDir` this chart mounts. The worker's
heartbeat file must therefore live inside that mount, and the chart — the layer
that owns the mount layout — is the only one that can state the path correctly.
`health.heartbeatFile` is projected into **both** workloads as
`ATOMI_HEALTH__HEARTBEATFILE` (R14/R21 env override tier), so the writer and the
prober can never disagree about where the heartbeat is.

## db-init ordering, and why the rollout stays a rolling update

`goconsumer-dbinit` carries **both** hook systems — helm
...
```sh
helm upgrade --install goconsumer infra/root_chart \
...
dash-less token (B30.4): `goconsumer-worker`, `goconsumer-dbinit`,
`goconsumer-config`, `goconsumer-secrets`. All submodules share one service
...
ONE semver is shared by the image tag and **both** charts' `version` fields —
chart `version` == image `Tag`, which is what Kargo aligns on. CI's one-semver
flow sets `appVersion` to the image version and packages at that same version,
and the sibling primordial chart participates identically. There is no carbon
bump, no umbrella/release-train, and no downstream dispatch chain.
...
| health | object | `{"command":["/go-consumer","health"],"heartbeatFile":"/tmp/worker-heartbeat.json","liveness":{"failureThreshold":3,"initialDelaySeconds":15,"periodSeconds":30,"timeoutSeconds":5},"readiness":{"failureThreshold":3,"initialDelaySeconds":5,"periodSeconds":10,"timeoutSeconds":5}}` | Dependency-blind liveness and readiness (DQ16/R20). BOTH probes exec the binary's `health` subcommand, which inspects worker heartbeat and internal state only — never postgres/redis/S3 reachability. |
| health.command | list | `["/go-consumer","health"]` | Exec command. Must invoke the health subcommand through the same artifact the Dockerfile entrypoint runs. |
| health.heartbeatFile | string | `"/tmp/worker-heartbeat.json"` | Heartbeat file both the worker and the probe address, projected as `ATOMI_HEALTH__HEARTBEATFILE`. It MUST sit inside the writable scratch mount: `containerSecurityContext.readOnlyRootFilesystem` is on, so the image's own filesystem cannot be written and a heartbeat under it would fail every probe. |
...
| image | object | `{"pullPolicy":"IfNotPresent","pullSecrets":[],"repository":"ghcr.io/atomicloud/diene.go-consumer","tag":""}` | Worker container image. The artifact's entrypoint is the compiled `/go-consumer` binary; the subcommand is supplied per workload through `worker.args` / `dbInit.args`. |
| image.pullPolicy | string | `"IfNotPresent"` | Image pull policy. |
| image.pullSecrets | list | `[]` | Optional image pull secrets. |
| image.repository | string | `"ghcr.io/atomicloud/diene.go-consumer"` | Image repository. Never `:latest` — the tag is a pin (M25). |
...
| podSecurityContext | object | `{"fsGroup":65532,"runAsGroup":65532,"runAsNonRoot":true,"runAsUser":65532,"seccompProfile":{"type":"RuntimeDefault"}}` | Pod-level security context. The image is unprivileged (M26) and the distroless `nonroot` base runs as uid/gid 65532. |
| secret | object | `{"enabled":true,"refreshInterval":"1h","serviceFolder":"/goconsumer","sharedFolder":"/shared","store":{"kind":"SecretStore","name":"platform-store"},"syncWave":"-2"}` | One service-scoped ExternalSecret materializing this service's secret from the platform SecretStore that carbon creates (R16 — the bromine subchart is dissolved). All submodules share the one service secret. |
| secret.enabled | bool | `true` | Render the ExternalSecret. |
| secret.refreshInterval | string | `"1h"` | ESO refresh cadence. |
| secret.serviceFolder | string | `"/goconsumer"` | Service-folder path fanned in with a `GOCONSUMER_` prefix. |
...
| serviceTree | object | `{"layer":"2","module":"worker","platform":"diene","service":"goconsumer"}` | Stable service-tree projection (LPSM). Landscape and cluster slots are added by the two independent overlay dimensions (`values.<landscape>.yaml` then `values.<cluster>.yaml`); the base file never carries them. |
