...
let
  ...
  validator-runtime = pkgs.buildEnv {
    ...
    paths = [
      packages.atomiutils
      # Flutter carries the same Dart SDK while also providing the tool the
      # auth-engine analysis, test, and package validators invoke.
      packages.flutter
      packages.git
    ];
  };
...
pre-commit-lib.run {
  ...
  hooks = {
    treefmt = {
      ...
      package = formatter;
      excludes = [
        # The C0 projection is digest-bound and must remain byte-identical.
