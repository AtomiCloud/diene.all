...
let
  ...
  go-deps = pkgs.buildGoModule {
    ...
    version = "0";
    src = ../.;
    vendorHash = "sha256-MUIvxFyGmCPCYjBnbBnt9sAaY/As8CqbTa6YLpxuGpQ=";
    proxyVendor = true;
  };
  ...
  go-deadcode-runtime = pkgs.buildEnv {
    name = "go-consumer-deadcode-runtime";
    paths = [
      packages.bash
      packages.deadcode
      packages.git
  ...
  go-deadcode = "${packages.bash}/bin/bash -c 'export PATH=${go-deadcode-runtime}/bin; export CGO_ENABLED=0; export GOPROXY=file://${go-deps.goModules}; export GOSUMDB=off; export GOMODCACHE=\"\${TMPDIR:-/tmp}/go-consumer-mod-cache\"; ${
    builtins.concatStringsSep " && " (
      map (component: "${packages.bash}/bin/bash ./scripts/local/${component}.sh") [
        "staticcheck-whole"
        "deadcode-whole"
