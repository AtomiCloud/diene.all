...
#
# This script runs from the `release.hooks.prepare` afterWrite step of releaser
# v2, which fires AFTER the changelog entry has been written and BEFORE the
# configured assets are committed — so the bytes that land in the release commit
# are already the bytes the hook set demands.
