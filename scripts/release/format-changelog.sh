...
#
# This script runs from `release.hooks.prepare` in schema v2. The parent renamed
# atomi_release.yaml to release.yaml and replaced the semantic-release plugin
# chain with releaser; the `afterWrite` phase is the same position the exec
# prepare step held — AFTER the changelog entry is written and BEFORE the commit
# assets are staged — so the bytes that land in the release commit are already
# the bytes the hook set demands.

root_dir="$(git rev-parse --show-toplevel)"
cd "${root_dir}"

changelog="$(yq -r '.release.changelog.path // ""' release.yaml)"
[[ -z ${changelog} || ${changelog} == "null" ]] && echo "❌ release.yaml declares no release.changelog.path" >&2 && exit 1
