...
#
# This script runs from the release.hooks.prepare step. The prepare hooks run
# AFTER the changelog entry has been written and BEFORE the commit step stages
# release.commit.assets, so the bytes that land in the release commit are
# already the bytes the hook set demands.
