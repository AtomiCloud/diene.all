...
[ -f "${license_file}" ] || {
  echo "❌ root '${license_file}' is missing" >&2
  exit 1
}

declared="$(jq -r '.license // ""' "${manifest}")"
[ -n "${declared}" ] || {
  echo "❌ '${manifest}' declares no license" >&2
  exit 1
}
...
[ -z "${license_kind}" ] && {
  echo "❌ could not recognise the license kind from '${license_file}'" >&2
  exit 1
}

[ "${declared}" != "${license_kind}" ] && {
  echo "❌ manifest license '${declared}' != root LICENSE kind '${license_kind}'" >&2
  exit 1
}
