  ...
  jq -n \
    ...
    '{
      ...
      deviations: [
        ...
        "before L5 the throwaway harness scales down the Application controller, clears the resources finalizer only on platform-sitother, deletes it, and triggers its owning ApplicationSet with the one-shot refresh annotation; this recreates an operation-free UID and clears the intentionally CRD-light initial retry before the tag moves",
