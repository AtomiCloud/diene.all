...
# a VERSION file plus the member manifest at packages/diene_interfaces/pubspec.yaml.
fixture="$(mktemp -d)"
...
rg -q 'packages/diene_interfaces/CHANGELOG.md' release.yaml || {
  echo "❌ packages/diene_interfaces/CHANGELOG.md is absent from releaser assets" >&2
  exit 1
}
rg -q 'packages/diene_interfaces/pubspec.yaml' release.yaml || {
  echo "❌ packages/diene_interfaces/pubspec.yaml is absent from releaser assets" >&2
  exit 1
}
