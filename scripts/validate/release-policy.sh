...
rg -q 'packages/diene_result/CHANGELOG.md' release.yaml || {
  echo "❌ packages/diene_result/CHANGELOG.md is absent from releaser assets" >&2
  exit 1
}
rg -q 'packages/diene_result/pubspec.yaml' release.yaml || {
  echo "❌ packages/diene_result/pubspec.yaml is absent from releaser assets" >&2
  exit 1
}
