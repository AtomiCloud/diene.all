...
# a VERSION file plus the member manifest at packages/diene_auth_engine/pubspec.yaml.
fixture="$(mktemp -d)"
...
rg -q 'packages/diene_auth_engine/CHANGELOG.md' release.yaml || {
  echo "❌ packages/diene_auth_engine/CHANGELOG.md is absent from releaser assets" >&2
  exit 1
}
rg -q 'packages/diene_auth_engine/pubspec.yaml' release.yaml || {
  echo "❌ packages/diene_auth_engine/pubspec.yaml is absent from releaser assets" >&2
  exit 1
}
...
[[ -z ${changelog_title} ]] && echo "❌ release.yaml declares no changelog title" >&2 && exit 1

# The releaser reuses the title only when the file literally starts with it, so
