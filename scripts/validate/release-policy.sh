...
rg -q 'packages/diene_interfaces/CHANGELOG.md' atomi_release.yaml || {
  echo "❌ packages/diene_interfaces/CHANGELOG.md is absent from semantic-release assets" >&2
  exit 1
}
rg -q 'packages/diene_interfaces/pubspec.yaml' atomi_release.yaml || {
  echo "❌ packages/diene_interfaces/pubspec.yaml is absent from semantic-release assets" >&2
  exit 1
}
