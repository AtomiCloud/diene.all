#!/usr/bin/env bash
#
# PORTED VERBATIM from lib/dart/api-engine (scripts/validate/publish-archive.sh),
# which authored it; the lead's R-E33 brief directs this node to USE that gate
# rather than write its own. Only three package-specific values differ from the
# source: member_dir, the must_exclude list, and the barrel filenames. The logic,
# the refusal-on-empty guards and the correction note below are unmodified.
#
# This is a per-node COPY because the gate has not yet been promoted to the
# parent. Promotion to dart-lib so every dart lib inherits ONE copy is a recorded
# parent obligation — see exec/nodes/lib__dart__auth-engine/node.json ->
# evidence.parent_obligations[publish-archive-gate-promotion]. On cascade,
# resolve in favour of the parent and delete this copy.
#
...
dangling=0
checked=0
for barrel in lib/diene_auth_engine.dart lib/test_helper.dart; do
  [ -f "${member_dir}/${barrel}" ] || {
