...
# partial-input, stale-tracked, new-untracked, the Nix-builder preserve
# fallback (complete, incomplete, drift, and a HEAD-less staged checkout), and the
# COLD-MODULE-CACHE ordering contract of every normal setup entry point.
#
# Nothing here touches the network: the cold cache is a stub `go` that reports a
# resolved module directory only after `go mod download` has run.

repo_root="$(cd "$(dirname "$0")/../.." && pwd)"
sync_src="${repo_root}/scripts/local/skills-sync.sh"
fresh_src="${repo_root}/scripts/validate/skills-freshness.sh"
setup_src="${repo_root}/scripts/ci/setup.sh"
taskfile_src="${repo_root}/Taskfile.yaml"
...
# 15. COLD MODULE CACHE. Dependency-skill regeneration reads the RESOLVED module
# directory of each dependency, so every normal setup entry point must download
# the Go dependencies BEFORE running the regenerator. On a cold cache the module
# graph is known but `go list -m -json all` reports no ".Dir", and skills-sync
# correctly fails closed — so a regenerate-first order breaks the normal path on
# any fresh runner while passing on a warm one. The stub `go` below reproduces
# exactly that: no ".Dir" until `go mod download` has run.
cold_repo() {
  local dir
  dir="$(new_repo "$1")"
  mkdir -p "${dir}/bin" "${dir}/scripts/ci"
  cp "${setup_src}" "${dir}/scripts/ci/setup.sh"
  cat >"${dir}/bin/go" <<EOF
#!/usr/bin/env bash
resolved="${dir}/cache/.resolved"
...
JSON
    ...
    cat <<JSON
{"Path":"github.com/AtomiCloud/diene.example","Main":true}
{"Path":"github.com/AtomiCloud/diene.go-core-utils"}
{"Path":"github.com/AtomiCloud/diene.go-errors-problems"}
JSON
  fi
fi
EOF
  chmod +x "${dir}/bin/go" "${dir}/scripts/ci/setup.sh" "${dir}/scripts/local/skills-sync.sh"
  echo "${dir}"
}

# entrypoint writes an abort-on-failure script from the command lines an entry
# point actually declares, so the recorded exit code is the FIRST failure's, the
# way both `task` and `set -e` behave.
entrypoint() {
  local dir="$1"
  {
...
# run_cold executes one shell command line in a cold repo with the stub `go`
# first on PATH, exactly as a fresh runner would resolve it.
run_cold() {
  local dir="$1" command_line="$2" code=0
  (cd "${dir}" && env -i HOME="${HOME}" PATH="${dir}/bin:${toolbin}" bash -c "${command_line}" >/dev/null 2>&1) || code=$?
  echo "${code}"
}

# regenerated reports whether the vendor tree holds both dependency skills.
regenerated() {
  [ -f "$(vendor "$1")/diene.go-core-utils/core-usage/SKILL.md" ] &&
    [ -f "$(vendor "$1")/diene.go-errors-problems/errors-usage/SKILL.md" ]
}

# The Taskfile entry points are executed as authored rather than restated here,
# so a future reordering of their commands is what this case actually measures.
if ! command -v yq >/dev/null 2>&1; then
  fail "cold-cache Taskfile cases need yq to read the authored setup commands"
else
  for task_name in setup skills:sync; do
    dir="$(cold_repo "cold-task-${task_name//:/-}")"
    yq -r ".tasks.\"${task_name}\".cmds[]" "${taskfile_src}" | entrypoint "${dir}"
    code="$(run_cold "${dir}" "bash entry.sh")"
    if [ "${code}" = 0 ] && regenerated "${dir}"; then
      pass "Taskfile ${task_name} resolves Go dependencies before regenerating on a cold cache"
    else
      fail "Taskfile ${task_name} must resolve before regenerating on a cold cache (code=${code})"
    fi
  done
fi

dir="$(cold_repo cold-ci-setup)"
code="$(run_cold "${dir}" "bash scripts/ci/setup.sh")"
if [ "${code}" = 0 ] && regenerated "${dir}"; then
  pass "scripts/ci/setup.sh resolves Go dependencies before regenerating on a cold cache"
else
  fail "scripts/ci/setup.sh must resolve before regenerating on a cold cache (code=${code})"
fi

# 16. The control for case 15: the regenerate-first order must still fail closed
# on the same cold repo. Without this the ordering cases could pass vacuously on
# a stub that resolved unconditionally.
dir="$(cold_repo cold-regenerate-first)"
printf './scripts/local/skills-sync.sh\ngo mod download\n' | entrypoint "${dir}"
before="$(snapshot "$(vendor "${dir}")")"
code="$(run_cold "${dir}" "bash entry.sh")"
after="$(snapshot "$(vendor "${dir}")")"
if [ "${code}" != 0 ] && [ "${before}" = "${after}" ]; then
  pass "regenerating before resolving still fails closed on a cold cache, without mutation"
else
  fail "a cold regenerate-first order must fail closed without mutation (code=${code})"
fi
