...
RG_OUTPUT=''
rg_capture() { # <rg args...>; stores matching lines, flags a real error
  local status=0
  set +e
  RG_OUTPUT="$(rg "$@" 2>&1)"
  status=$?
  ...
  if [[ ${status} -eq 1 ]]; then
    ...
    bad "ripgrep failed with status ${status}: rg $*: ${RG_OUTPUT}"
    RG_OUTPUT=''
  fi
}
...
rg_capture --no-filename --count-matches \
  -e '\bdeepMerge(All)?\s*\([^)]*\)\s*(=>|\{)' \
  -e '\bcanonicalConfigKey\s*\([^)]*\)\s*(=>|\{)' \
  -e '\bcoerceEnvironmentScalar\s*\([^)]*\)\s*(=>|\{)' \
  -e '\benvironmentToNestedMap\s*\([^)]*\)\s*(=>|\{)' \
  --glob '*.dart' "${member_dir}/lib"
redefinitions=0
while IFS= read -r count; do
  [[ -z ${count} ]] && continue
  if [[ ${count} =~ ^[0-9]+$ ]]; then
    redefinitions=$((redefinitions + count))
  else
    bad "ripgrep returned a non-numeric definition count: ${count}"
  fi
done <<<"${RG_OUTPUT}"
...
for dep in diene_core_utils diene_problems diene_result; do
  ...
  # a ripgrep error.
  rg_capture -l "^import 'package:${dep}/" --glob '*.dart' "${member_dir}/lib"
  imports=0
  while IFS= read -r path; do
    [[ -n ${path} ]] && imports=$((imports + 1))
  done <<<"${RG_OUTPUT}"
