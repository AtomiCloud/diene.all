...
package_ids=(AtomiCloud.Diene.Interfaces AtomiCloud.Diene.Interfaces.TestHelper)
...
if [ "${mode}" = "metadata" ]; then
  for package_id in "${package_ids[@]}"; do
    ...
    [ "$(xmlstarlet sel -N n="${namespace}" -t -v '/n:package/n:metadata/n:projectUrl' "${nuspec}")" != "https://github.com/AtomiCloud/diene.dotnet-interfaces" ] && echo "❌ ${package_id} project URL metadata missing" >&2 && exit 1
    [ "$(xmlstarlet sel -N n="${namespace}" -t -v '/n:package/n:metadata/n:repository/@url' "${nuspec}")" != "https://github.com/AtomiCloud/diene.dotnet-interfaces" ] && echo "❌ ${package_id} repository metadata missing" >&2 && exit 1
    contents="$(unzip -Z1 "${package}")"
    for required in README.md logo.png LICENSE skills/diene-dotnet-interfaces-usage/SKILL.md; do
