...
domain_in_controllers="$(grep -rnE '"github\.com/AtomiCloud/diene\.boron/lib/operator/boron"' adapters/operator/controllers || true)"
[ -n "${domain_in_controllers}" ] && echo "${domain_in_controllers}" >&2 && echo "❌ operator architecture: controllers must not import lib/operator/boron; route domain decisions through lib/operator/reconcile" >&2 && exit 1
