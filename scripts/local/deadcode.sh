...
(
  cd "${member_dir}"
  flutter pub run dart_code_linter:metrics check-unused-code lib test example
  flutter pub run dart_code_linter:metrics check-unused-files lib test example
)
...
# Standalone manifest: drop `resolution: workspace` so `flutter pub get` resolves
