...
cluster_name="${K3D_CLUSTER_NAME:-diene-lithium}"
registry_name="${K3D_REGISTRY_NAME:-diene-lithium-registry}"
...
# Every public path (Taskfile, proof wrapper, direct invocation) must present the
# same per-invocation ownership/isolation contract: no marker means no cluster.
[ "${K3D_ISOLATE_BY_PATH:-}" = "true" ] || {
  echo "❌ K3D_ISOLATE_BY_PATH=true is mandatory to create a k3d cluster" >&2
  exit 1
}
[ -n "${ownership_marker}" ] || {
