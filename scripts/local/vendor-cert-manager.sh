#!/usr/bin/env bash
# Vendor the pinned upstream cert-manager chart (pure passthrough — no patch).
...
version="$(yq -r '.dependencies[] | select(.name == "cert-manager") | .version' "${chart_dir}/Chart.yaml")"
...
archive="${chart_dir}/charts/cert-manager-${version}.tgz"
tmp="$(mktemp -d)"
trap 'rm -rf "${tmp}"' EXIT
...
[ ! -s "${archive}" ] && echo "❌ cert-manager archive ${archive} is missing" >&2 && exit 1
...
archive_version="$(yq -r '.version' "${tmp}/cert-manager/Chart.yaml")"
archive_app="$(yq -r '.appVersion' "${tmp}/cert-manager/Chart.yaml")"
...
echo "✅ cert-manager ${version}/${expected_app} dependency vendored (pure passthrough, upstream archive)"
