...
/// This member's own root (`packages/diene_problems`), derived from the script
/// so the generator behaves identically however it is invoked.
final Directory _packageRoot = File.fromUri(Platform.script).parent.parent;

/// The workspace root, which OWNS the frozen C0 release. The release is
/// repository-level (one release, inherited by every member — R-E8a), while
/// every generated artifact belongs to this member.
final Directory _repositoryRoot = _packageRoot.parent.parent;

/// A release-owned input, resolved from the repository root.
File _repositoryFile(String path) => File('${_repositoryRoot.path}/$path');

/// A generated output, resolved from this member's root.
...
_Release _loadRelease() {
  final File manifestFile = _repositoryFile(_manifestPath);
  ...
  final File policyFile = _repositoryFile(formatterPolicy['path']! as String);
  ...
  final List<int> sumsBytes = _repositoryFile(_sumsPath).readAsBytesSync();
...
Map<String, Object?> _loadCaseFile() {
  final File file = _repositoryFile(_casePath);
...
void _validateManifest(Map<String, Object?> value) {
  ...
  for (int index = 0; index < secondary.length; index += 1) {
    final String label = 'secondarySources[$index]';
    final Map<String, Object?> pin = _map(secondary[index], label);
    _expectKeys(pin, <String>{'path', 'sha256'}, label);
    _expect(
      pin['path'] == expectedSecondaryPaths[index],
      'wrong $label path: expected ${expectedSecondaryPaths[index]}',
    );
    _expect(
      RegExp(
        r'^[0-9a-f]{64}$',
      ).hasMatch(_string(pin['sha256'], '$label.sha256')),
...
void _validateSums(List<int> bytes) {
  ...
  for (final MapEntry<String, String> entry in expectedDigests.entries) {
    final File file = _repositoryFile('$_c0Root/${entry.key}');
...
List<String> _eligibleFiles(String directory, String extension) {
  final String prefix = '${_repositoryRoot.path}/$_c0Root/';
  return Directory('${_repositoryRoot.path}/$_c0Root/$directory')
...
void _validateCaseFile(Map<String, Object?> value) {
  ...
  _expect(
    _list(cases['rfc9457Members'], 'rfc9457Members').isNotEmpty,
    'missing RFC 9457 members',
  );
  _expect(
    _list(cases['extensions'], 'extensions').isNotEmpty,
    'missing extensions',
  );
  _expect(
...
Map<String, List<int>> _renderOutputs(
  ...
  final Map<String, List<int>> outputs = <String, List<int>>{
    'lib/src/c0_problem_contract.g.dart': utf8.encode(
      _renderDart(release, caseFile),
    ),
    'test/fixtures/c0/envelope.json': utf8.encode(
      _prettyCanonical(<String, Object?>{
        r'$generated': generated,
        'extensions': cases['extensions'],
        'invalid': envelopes['invalid'],
        'rfc9457Members': cases['rfc9457Members'],
        'valid': envelopes['valid'],
      }),
    ),
    'test/fixtures/c0/catalog-entry.json': utf8.encode(
      _prettyCanonical(<String, Object?>{
        r'$generated': generated,
        'requiredFields': catalog['shape'],
        'samples': catalog['samples'],
      }),
    ),
    'test/fixtures/c0/type-uri.json': utf8.encode(
      _prettyCanonical(<String, Object?>{
        r'$generated': generated,
        'invalid': typeUri['invalid'],
        'template': cases['typeUriTemplate'],
        'valid': typeUri['valid'],
      }),
    ),
  };
...
String _renderDart(_Release release, Map<String, Object?> caseFile) {
  ...
  final String secondaryPins = secondary
      .map((Object? value) {
        ...
        return 'C0ProblemSourcePin(path: ${_dartString(pin['path']! as String)}, '
...
final class C0ProblemSourcePin {
  const C0ProblemSourcePin({required this.path, required this.sha256, this.note});
  final String path;
  final String sha256;
  final String? note;
}
...
const C0ProblemContract c0ProblemContract = C0ProblemContract(
  provenance: C0ProblemReleaseProvenance(
    ...
    secondarySources: <C0ProblemSourcePin>[$secondaryPins],
  ...
  rfc9457Members: ${_dartStringList(_list(cases['rfc9457Members'], 'rfc9457Members').cast<String>())},
  extensions: ${_dartStringList(_list(cases['extensions'], 'extensions').cast<String>())},
  typeUriTemplate: ${_dartString(cases['typeUriTemplate']! as String)},
  typeUri: ${_dartLiteral(cases['typeUri'])},
  envelopes: ${_dartLiteral(cases['envelopes'])},
  catalogEntry: ${_dartLiteral(cases['catalogEntry'])},
);
''';
}
...
void _materialize(Map<String, List<int>> outputs, {required bool check}) {
  ...
  try {
    ...
    final String dartPath =
        '${temporary.path}/lib/src/c0_problem_contract.g.dart';
    ...
    for (final String path in outputs.keys) {
      ...
      final File committed = _packageFile(path);
      if (check) {
  ...
  stdout.writeln(
    check
        ? 'C0 Problem projection matches the authenticated release'
        : 'generated C0 Problem projection from the authenticated release',
  );
}
