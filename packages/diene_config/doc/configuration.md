...
Retain the exact `ConfigBlock<T>` instances passed to `ConfigSchema` and use
those same objects for typed access. Slice membership is identity-based: a new
same-key block was not composed, so `hasSlice` returns `false`, `optionalSlice`
returns `None`, and `slice` throws `StateError`. This prevents a lookalike block
with a different decoder or type from reading another block's decoded value.
...
projection. Before generating, the tool authenticates the exact normative case
bytes through the release-wide ledger and re-derived frozen release digest. The
fixture carries those witnesses, and the conformance suite re-derives the same
chain from the vectors it actually drives. Changing the normative release
either flows through as a newly accepted release identity or reddens the
`--check` gate; it cannot be diverged from by editing an assertion or merely
regenerating a local checksum.
